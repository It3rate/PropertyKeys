...
namespace DataArcs.Tests
{
	...
	public class IndexFloatConversionTests
	{
		...
		public void TestGetStrideTsForIndex()
		{
			...
			sample = SamplerUtils.GetStrideTsForIndex(virtualCount, strides, 0);
			ArraysEqual(new float[] {0, 0}, sample, delta);
			sample = SamplerUtils.GetStrideTsForIndex(virtualCount, strides, 1);
			ArraysEqual(new float[] {1f / cl, 0}, sample, delta);
			sample = SamplerUtils.GetStrideTsForIndex(virtualCount, strides, 9);
			ArraysEqual(new float[] {1f, 0}, sample, delta);
			sample = SamplerUtils.GetStrideTsForIndex(virtualCount, strides, 10);
			ArraysEqual(new float[] {0, 1f / cl}, sample, delta);
			sample = SamplerUtils.GetStrideTsForIndex(virtualCount, strides, 11);
			ArraysEqual(new float[] {1f / cl, 1f / cl}, sample, delta);
			sample = SamplerUtils.GetStrideTsForIndex(virtualCount, strides, 53);
			ArraysEqual(new float[] {3f / cl, 5f / cl}, sample, delta);
			sample = SamplerUtils.GetStrideTsForIndex(virtualCount, strides, 99);
			ArraysEqual(new float[] {9f / cl, 9f / cl}, sample, delta);
			sample = SamplerUtils.GetStrideTsForIndex(virtualCount, strides, 100);
			ArraysEqual(new float[] {9f / cl, 9f / cl}, sample, delta);
			sample = SamplerUtils.GetStrideTsForIndex(virtualCount, strides, 101);
			ArraysEqual(new float[] {9f / cl, 9f / cl}, sample, delta);
		}
		...
		public void TestGetStrideTsForT()
		{
			...
			float[] sample;
			sample = SamplerUtils.GetStrideTsForT(virtualCount, strides, -3f);
			ArraysEqual(new float[] {0, 0}, sample, delta);
			sample = SamplerUtils.GetStrideTsForT(virtualCount, strides, 0);
			ArraysEqual(new float[] {0, 0}, sample, delta);
			sample = SamplerUtils.GetStrideTsForT(virtualCount, strides, 1f / len);
			ArraysEqual(new float[] {1f / cl, 0}, sample, delta);
			sample = SamplerUtils.GetStrideTsForT(virtualCount, strides, 9f / len);
			ArraysEqual(new float[] {1f, 0}, sample, delta);
			sample = SamplerUtils.GetStrideTsForT(virtualCount, strides, 10f / len);
			ArraysEqual(new float[] {0, 1f / cl}, sample, delta);
			sample = SamplerUtils.GetStrideTsForT(virtualCount, strides, 11f / len);
			ArraysEqual(new float[] {1f / cl, 1f / cl}, sample, delta);
			sample = SamplerUtils.GetStrideTsForT(virtualCount, strides, 53f / len);
			ArraysEqual(new float[] {3f / cl, 5f / cl}, sample, delta);
			sample = SamplerUtils.GetStrideTsForT(virtualCount, strides, 1f);
			ArraysEqual(new float[] {9f / cl, 9f / cl}, sample, delta);
			sample = SamplerUtils.GetStrideTsForT(virtualCount, strides, 100f / len);
			ArraysEqual(new float[] {9f / cl, 9f / cl}, sample, delta);
			sample = SamplerUtils.GetStrideTsForT(virtualCount, strides, 101 / len);
			ArraysEqual(new float[] {9f / cl, 9f / cl}, sample, delta);
		}
