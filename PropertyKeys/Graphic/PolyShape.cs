...
namespace DataArcs.Graphic
{
    public class PolyShape : GraphicBase
    {
        ...
        {
            var orientation = Orientation.GetValueAtT(orientationT)[0];
            var roundness = Roundness.GetValueAtT(roundnessT)[0];
            var radius = Radius.GetValueAtT(radiusT).FloatData;
            var radiusX = radius[0];
            var radiusY = radius.Length > 1 ? radius[1] : radius[0];
            var starness = Starness.GetValueAtT(starnessT)[0];
            var pointCount = PointCount.GetValueAtT(pointCountT).IntDataAt(0);
