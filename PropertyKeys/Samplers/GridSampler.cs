...
namespace DataArcs.Samplers
{
	public class GridSampler : Sampler
	{
        ...
        {
			Strides = strides;
            SliceCount = strides[0];
            for (int i = 1; i < strides.Length; i++)
            {
                if (strides[i] != 0)
                {
                    SliceCount *= strides[i];
                }
                else
                {
                    break;
        ...
        public override Series GetNeighbors(Series series, int index, bool wrapEdges = true)
        {
            ...
            result.SetRawDataAt(0, series.GetVirtualValueAt(WrappedIndexes(indexX + 1, indexY), SliceCount));
            result.SetRawDataAt(1, series.GetVirtualValueAt(WrappedIndexes(indexX, indexY - 1), SliceCount));
            result.SetRawDataAt(2, series.GetVirtualValueAt(WrappedIndexes(indexX - 1, indexY), SliceCount));;
            result.SetRawDataAt(3, series.GetVirtualValueAt(WrappedIndexes(indexX, indexY + 1), SliceCount));
