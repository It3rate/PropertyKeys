...
namespace DataArcs.Samplers
{
	public class BezierSampler : Sampler
	{
        ...
        private Series GetSeriesAtT(float[] a, float[] b, BezierMove moveType, float t)
		{
			...
			switch (moveType)
			{
				...
				case BezierMove.LineTo:
					result[0] = a[0] + (b[p2Index] - a[0]) * t;
					result[1] = a[1] + (b[p2Index + 1] - a[1]) * t;
					break;
				case BezierMove.QuadTo:
					result[0] = it * it * a[0] + 2 * it * t * b[0] + t * t * b[p2Index];
					result[1] = it * it * a[1] + 2 * it * t * b[1] + t * t * b[p2Index + 1];
					break;
