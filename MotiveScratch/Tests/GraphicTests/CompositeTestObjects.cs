...
namespace Motive.Tests.GraphicTests
{
    ...
    public class CompositeTestObjects : ITestScreen
    {
        ...
        public CompositeTestObjects(Runner runner)
        {
	        _runner = runner;
        }
        ...
        private BlendTransition GetVersion(int index)
        {
            ...
            switch (index)
            {
	            ...
	            case 2:
		            ...
		            break;
                default:
                    comp = GetTest1(0, _runner.CurrentMs, 2000f);
                    break;
            }
        ...
        private void CompOnEndTransitionEvent(object sender, EventArgs e)
        {
            ...
            else if (_count < 2)
            {
                ...
                //BlendTransition newBT = new BlendTransition(bt, comp, 0, _runner.CurrentMs, 3000, easeStore);
                //_runner.ActivateElement(newBT);

                nextComp.End = nextComp.Start;
                nextComp.Start = lastComp;
                nextComp.Easing = easeStore;
                nextComp.GenerateBlends();
                _runner.ActivateComposite(nextComp.Id);
                lastComp = nextComp;
            }
            else
            {
